...
exp_dir_wE = '/home/lucy/spikingPC/results/Apr-17-2023/' + \
'conv_fptt_ener1.0_1llocalcov_chann[2]_lr0.001_batch256_convadpTrueksize16_readout50decay1e-4dt1_nobn_tauouttrainable_nosoftmax_withbias_pad410/'
exp_dir_woE = '/home/lucy/spikingPC/results/Apr-17-2023/' + \
'conv_fptt_ener0.0_1llocalcov_chann[2]_lr0.001_batch256_convadpTrueksize16_readout50decay1e-4dt1_nobn_tauouttrainable_nosoftmax_withbias_pad410/'
# define network
if '2llocal' in exp_dir_wE:
    model_wE = SnnLocalConvNet(IN_dim, hidden_channels, kernel_size, stride,
                               ...
                               pooling=pooling, is_rec=is_rec, dt=dt, bias=bias)
    model_woE = SnnLocalConvNet(IN_dim, hidden_channels, kernel_size, stride,
                                paddings, n_classes, is_adapt_conv=conv_adp,
                                dp_rate=dp, p_size=num_readout,
                                pooling=pooling, is_rec=is_rec, dt=dt, bias=bias)
elif '1llocal' in exp_dir_wE:
    model_wE = SnnLocalConvNet1Layer(IN_dim, hidden_channels, kernel_size, stride,
                               paddings, n_classes, is_adapt_conv=conv_adp,
                               dp_rate=dp, p_size=num_readout,
                               pooling=pooling, is_rec=is_rec, dt=dt)
    model_woE = SnnLocalConvNet1Layer(IN_dim, hidden_channels, kernel_size, stride,
                                paddings, n_classes, is_adapt_conv=conv_adp,
                                dp_rate=dp, p_size=num_readout,
                                pooling=pooling, is_rec=is_rec, dt=dt)
else:
...
hiddens_wE, preds_wE, images_all, _ = get_all_analysis_data(model_wE, test_loader, device, IN_dim, T, batch_no=batches,
                                                            occlusion_p=occlusion_p, conv=True)
...
########################################################################################################################
dig = 0
no_input = torch.zeros((1, IN_dim[0], IN_dim[1], IN_dim[2])).to(device)
with torch.no_grad():
    model_wE.eval()
    model_woE.eval()
...
for i, (data, target) in enumerate(test_loader):
    ...
    with torch.no_grad():
        ...
        l1_E = get_states_conv([h_E], 1, model_wE.conv1.output_shape, batch_size, T=T, num_samples=batch_size)
        l1_nE = get_states_conv([h_nE], 1,model_woE.conv1.output_shape, batch_size, T=T, num_samples=batch_size)

        # l2_E = get_states_conv([h_E], 5, model_wE.conv2.output_shape, batch_size, T=T, num_samples=batch_size)
        # l2_nE = get_states_conv([h_nE], 5, model_woE.conv2.output_shape, batch_size, T=T, num_samples=batch_size)

        l3_E = get_states([h_E], 5, model_wE.pop_enc.hidden_dim, batch_size, T=T, num_samples=batch_size)
        l3_nE = get_states([h_nE], 5, model_woE.pop_enc.hidden_dim, batch_size, T=T, num_samples=batch_size)
...
# avg all samples
l1_norm_E = l1_norm_E / len(test_loader.dataset)
l1_norm_nE = l1_norm_nE / len(test_loader.dataset)

# l2_norm_E = l2_norm_E / len(test_loader.dataset)
# l2_norm_nE = l2_norm_nE / len(test_loader.dataset)

l3_norm_E = l3_norm_E / len(test_loader.dataset)
l3_norm_nE = l3_norm_nE / len(test_loader.dataset)
...
for i in range(10):
    ...
    with torch.no_grad():
        ...
        #
        l1_E = get_states_conv([hidden_gen_E_], 1, model_wE.conv1.output_shape, 1, clamp_t, num_samples=1)
        l1_nE = get_states_conv([hidden_gen_nE_], 1, model_woE.conv1.output_shape, 1, clamp_t, num_samples=1)

        # get gen
        # l2_E = get_states_conv([hidden_gen_E_], 5, model_wE.conv2.output_shape, 1, clamp_t, num_samples=1)
        # l2_nE = get_states_conv([hidden_gen_nE_], 5, model_woE.conv2.output_shape, 1, clamp_t, num_samples=1)

        l3_E = get_states([hidden_gen_E_], 5, model_wE.pop_enc.hidden_dim, 1, clamp_t, num_samples=1)
        l3_nE = get_states([hidden_gen_nE_], 5, model_woE.pop_enc.hidden_dim, 1, clamp_t, num_samples=1)
...
for i in range(10):
    ...
    dist_l2_E.append(distance.cosine(l1_clamp_E[i].flatten(), l1_norm_E[i].flatten()))
    dist_l2_nE.append(distance.cosine(l1_clamp_nE[i].flatten(), l1_norm_nE[i].flatten()))
...
pair_dist_E_l2 = pairwise_distances(l1_clamp_E.reshape(10, -1), l1_norm_E.reshape(10, -1), metric='cosine')
pair_dist_nE_l2 = pairwise_distances(l1_clamp_nE.reshape(10, -1), l1_norm_nE.reshape(10, -1), metric='cosine')
...
sns.heatmap(pair_dist_E_l2, ax=axes[0, 0], vmin=0, vmax=1, cbar_ax=cbar_ax)
axes[0, 0].set_ylabel('clamped reps')
axes[0, 0].set_title('normal reps')
axes[0, 0].tick_params(left=False, bottom=False)

sns.heatmap(pair_dist_nE_l2, ax=axes[0, 1], vmin=0, vmax=1, cbar=False)
axes[0, 1].set_title('normal reps')
axes[0, 1].tick_params(left=False, bottom=False)

sns.heatmap(pair_dist_E_l3, ax=axes[1, 0], vmin=0, vmax=1, cbar=False)
axes[1, 0].set_ylabel('clamped reps')
axes[1, 0].tick_params(left=False, bottom=False)

sns.heatmap(pair_dist_nE_l3, ax=axes[1, 1], vmin=0, vmax=1, cbar=False)
...
# %%
plt.hist(l3_clamp_E.flatten(), label='clamp')
plt.hist(l3_clamp_nE.flatten(), label='norm')
plt.legend()
plt.show()
# %%
