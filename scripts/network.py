...
class SNN_rec_cell(nn.Module):
    def __init__(self, input_size, hidden_size, is_rec=False, is_LTC=True, isAdaptNeu=True, oneToOne=False):
        ...
        # create self-recurrence mask
        self.self_rec_mask = torch.eye(hidden_size).to(device)
        self.self_rec_mask = 1 - self.self_rec_mask
    ...
    def forward(self, x_t, mem_t, spk_t, b_t):
        if self.is_rec:
            ...
            self.layer1_x.weight.data = self.layer1_x.weight * self.self_rec_mask
