...
class SnnNetwork2Layer(SnnNetwork):
    def __init__(
        ...
        self.top_down = nn.Linear(hidden_dims[2], hidden_dims[0])  # r2 to p1
        self.bottom_up = nn.Linear(hidden_dims[0], hidden_dims[2])  # p1 to r2
    ...
    def forward(self, x_t, h):
        ...
        mem_r1, spk_r1, curr_r1, b_r1 = self.r_in_rec(r_input1, mem_t=h[0], spk_t=h[1], curr_t=h[2], b_t=h[3])

        p_input1 = self.rin2rout1(spk_r1) + self.top_down(h[9])

        mem_p1, spk_p1, curr_p1, b_p1 = self.r_out_rec(p_input1, mem_t=h[4], spk_t=h[5], curr_t=h[6], b_t=h[7])

        r_input2 = self.rout2rin2(h[13]) + self.bottom_up(h[spk_p1])

        mem_r2, spk_r2, curr_r2, b_r2 = self.r_in_rec(r_input2, mem_t=h[8], spk_t=h[9], curr_t=h[10], b_t=h[11])

        p_input2 = self.rin2rout(spk_r2)

        mem_p2, spk_p2, curr_p2, b_p2 = self.r_out_rec(p_input2, mem_t=h[12], spk_t=h[13], curr_t=h[14], b_t=h[15])
