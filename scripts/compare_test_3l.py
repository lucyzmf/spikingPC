...
# for i in range(len(dp)):
    # define network
model_wE = SnnNetwork2Layer(IN_dim, hidden_dim, n_classes, is_adapt=adap_neuron, one_to_one=onetoone, dp_rate=0.4, is_rec=is_rec)
model_wE.to(device)

# define network
model_woE = SnnNetwork2Layer(IN_dim, hidden_dim, n_classes, is_adapt=adap_neuron, one_to_one=onetoone, dp_rate=0.4, is_rec=is_rec)
model_woE.to(device)
...
# get mean curr by class to compare between groups 
a_curr_l3_wE = get_states(hiddens_wE, 10, hidden_dim[2], batch_size, num_samples=n_samples, T=T)
a_curr_l3_woE = get_states(hiddens_woE, 10, hidden_dim[2], batch_size, num_samples=n_samples, T=T)

soma_l3_wE = get_states(hiddens_wE, 8, hidden_dim[2], batch_size, num_samples=n_samples, T=T)
soma_l3_woE = get_states(hiddens_woE, 8, hidden_dim[2], batch_size, num_samples=n_samples, T=T)

error_l3_wE = (a_curr_l3_wE - soma_l3_wE)**2
error_l3_woE = (a_curr_l3_woE - soma_l3_woE) **2
...
first_stim_t = int(T/4)
first_dig = 1
second_dig = 7
zeros = (target_all==first_dig).nonzero(as_tuple=True)[0]
ones = (target_all==second_dig).nonzero(as_tuple=True)[0]
...
with torch.no_grad():
    ...
    log_sm_E1, hidden1_E = model_wE.inference(images_all[sample_image_nos[0], :, :].view(-1, IN_dim).to(device), hidden_i, first_stim_t)
    log_sm_nE1, hidden1_nE = model_woE.inference(images_all[sample_image_nos[0], :, :].view(-1, IN_dim).to(device), hidden_i, first_stim_t)
    ...
    log_sm_E2, hidden2_E = model_wE.inference((images_all[sample_image_nos[1], :, :]).view(-1, IN_dim).to(device), hidden1_E[-1], (T-first_stim_t))
    log_sm_nE2, hidden2_nE = model_woE.inference((images_all[sample_image_nos[1], :, :]).view(-1, IN_dim).to(device), hidden1_nE[-1], (T-first_stim_t))
...
def usi(expected_curr, unexpected_curr, layer_idx, dig=first_dig):
    df = pd.DataFrame(np.vstack((expected_curr[target_all == dig].mean(axis=0).T, unexpected_curr.mean(axis=0).T)), \
    ...
    df_usi = pd.DataFrame({
        ...
        'usi': (df['mean a'][df['condition']=='normal seq'].to_numpy() - \
            df['mean a'][df['condition']=='stim change seq'].to_numpy()) / \
...
usi(a_curr_l2_wE, conti_a_curr_l2_E, 1)
usi(a_curr_l2_woE, conti_a_curr_l2_nE, 1)

# %%
usi(soma_l2_wE, conti_soma_l2_E, 1)
usi(soma_l2_woE, conti_soma_l2_nE, 1)
...
def create_error_df(error_norm, error_conti, high_var_map, layer_index, class_filter):
    ...
    df = pd.melt(df, id_vars=['t', 'condition'], value_vars=['neuron%i' %i for i in range(hidden_dim[layer_index])], var_name='neuron index', value_name='epsilon')
    df['high var'] = 0
...
### layer3
## w E model 
conti_a_curr_l3_E = get_states([continuous_seq_hiddens_E], 10, hidden_dim[2], batch_size=conti_samplesize, num_samples=conti_samplesize, T=T).squeeze()
conti_soma_l3_E = get_states([continuous_seq_hiddens_E], 8, hidden_dim[2], batch_size=conti_samplesize, num_samples=conti_samplesize, T=T).squeeze()

conti_error_l3_E = (conti_a_curr_l3_E - conti_soma_l3_E)**2

# w/o E model
conti_a_curr_l3_nE = get_states([continuous_seq_hiddens_nE], 10, hidden_dim[2], batch_size=conti_samplesize, num_samples=conti_samplesize, T=T).squeeze()
conti_soma_l3_nE = get_states([continuous_seq_hiddens_nE], 8, hidden_dim[2], batch_size=conti_samplesize, num_samples=conti_samplesize, T=T).squeeze()
...
def match_mismatch_ex(match_condition):
    ...
    with torch.no_grad():
        ...
        _, h2_E = model_wE.clamped_generate(clamp_class, zeros.view(-1, hidden_dim[0]), h1_E[-1], match_t, clamp_value=1, batch=True)
        _, h2_nE = model_woE.clamped_generate(clamp_class, zeros.view(-1, hidden_dim[0]), h1_nE[-1], match_t, clamp_value=1, batch=True)
        h_E += h2_E
        h_nE += h2_nE
...
# %%
# with energy
spk_mis_l3 = get_states([h_mismatch_E], 9, hidden_dim[2], len(h_mismatch_E[0][0]), T, num_samples=len(h_mismatch_E[0][0]))
spk_match_l3 = get_states([h_match_E], 9, hidden_dim[2], len(h_mismatch_E[0][0]), T, num_samples=len(h_mismatch_E[0][0]))
...
spk_mis_l3 = get_states([h_mismatch_nE], 9, hidden_dim[2], len(h_mismatch_nE[0][0]), T, num_samples=len(h_mismatch_nE[0][0]))
spk_match_l3 = get_states([h_match_nE], 9, hidden_dim[2], len(h_mismatch_nE[0][0]), T, num_samples=len(h_mismatch_nE[0][0]))
...
with torch.no_grad():
    ...
    log_sm_E_sup, hidden_sup_E = model_wE.inference(rep_seq.view(seq_len, -1, IN_dim).to(device), hidden_i, seq_len, bystep=True)
    log_sm_nE_sup, hidden_sup_nE = model_woE.inference(rep_seq.view(seq_len, -1, IN_dim).to(device), hidden_i, seq_len, bystep=True)
torch.cuda.empty_cache()
